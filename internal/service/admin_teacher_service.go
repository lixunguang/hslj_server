...
func AddTeacher(ctx *gin.Context, param dto.AddTeacherParam) (dto.AdminRes, cerror.Cerror) {
...
func AllTeacher(ctx *gin.Context) ([]dto.TeacherRes, cerror.Cerror) {

	var res []dto.TeacherRes

	allRes, err := dao.AllTeacher(ctx)

	for _, val := range allRes {
		var item dto.TeacherRes

...
func AddTeacher(ctx *gin.Context) {
	...
	//调用service
	res, err := service.AddTeacher(ctx, param)
...
func AllTeacher(ctx *gin.Context) {
	...
	//调用service
	res, err := service.AllTeacher(ctx)
...
func DelTeacher(ctx *gin.Context) {
	...
	//调用service
	res, err := service.DelTeacher(ctx, param.LoginID)
...
func UpdateTeacher(ctx *gin.Context) {
	...
	//获取参数
	var param dto.UpdateTeacherParam
	if err := ctx.ShouldBindJSON(&param); err != nil {
		logger.Errorc(ctx, "[%s] bind param fail,err=%+v", "method", err)
		util.FailJson(ctx, cerror.InvalidParams)
		return
	...
	//调用service
	res, err := service.UpdateTeacher(ctx, param)
